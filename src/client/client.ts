import {
   ...
   * const client = new A2AClient('https://agent.example.com', '.well-known/agent-card.json', 'your-bearer-token');
   * 
   * // Rotate token during runtime
   * client.rotateToken('new-bearer-token');
   */
  /**
  ...
  export class A2AClient {
    ...
    private bearerToken?: string; // Bearer token for authentication
     ...
     */
    constructor(agentBaseUrl: string, agentCardPath: string = AGENT_CARD_PATH, bearerToken?: string) {
     ...
     * @param newToken The new bearer token to use for subsequent requests
     */
    public rotateToken(newToken: string): void {
      this.bearerToken = newToken;
    }

    /**
     * Builds headers for HTTP requests, including authentication if a bearer token is set.
     * @param additionalHeaders Additional headers to include in the request
     * @returns Headers object with authentication and additional headers
     */
